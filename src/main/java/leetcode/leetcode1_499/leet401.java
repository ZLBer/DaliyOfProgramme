...
public class leet401 {
    ...
    //mine
  static   int []time=new int[]{1,2,4,8,16,32};
   static public List<String> readBinaryWatch(int num) {
List<String> result=new ArrayList<>();


for(int hourCount=0;hourCount<=num;hourCount++){
    int minutesCount=num-hourCount;
    List<Integer> hour=new ArrayList<>();
    List<Integer> minutes=new ArrayList<>();
helper(hourCount,hour,0,3,0);
helper(minutesCount,minutes,0,5,0);
    for(Integer h:hour){
        if(h>11) continue;
...
return  result;
  ...
  static   void helper(int count,List<Integer> list,int depth,int maxDepth,int curTime){
         if(count==0) {
             list.add(curTime);
             return;
         }
   ...
   //取该点
         helper(count-1,list,depth+1,maxDepth,curTime+time[depth]);
     //不取该点
         helper(count,list,depth+1,maxDepth,curTime);
    }
